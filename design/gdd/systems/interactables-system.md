...
- [ ] 普通按钮被任意顶面箱子占据时激活，箱子离开后释放
- [ ] 重压按钮仅被重压面箱子激活，其余顶面不触发
- [ ] 能源槽仅被能源面箱子供电
- [ ] 按钮激活/释放时，关联的所有门同步开启/关闭
- [ ] 门关闭时占据网格格子，阻挡玩家和箱子通行
- [ ] 门打开时让出网格格子，允许通行
- [ ] 门不会在有实体占据其格子时强行关闭
- [ ] 出口终端在 `requires_external_power=true` 时需要能源槽供能才可激活
- [ ] 出口终端在 `requires_external_power=false` 时玩家踩上即完成
- [ ] 出口激活后不可逆，即使后续断电也不撤销
- [ ] 每个机关的视觉状态（灯光颜色、压板位置、缩放）与逻辑状态一致
- [ ] 所有机关音效在正确时机播放，无遗漏无重复
- [ ] 关卡重置后所有机关恢复初始状态
- [ ] 玩家仅凭视觉即可区分：未触发/已触发按钮、开/关门、供能/未供能插槽、可用/不可用出口
