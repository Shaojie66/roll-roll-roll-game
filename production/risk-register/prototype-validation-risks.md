...
| R-003 | Level 5 requires too much route inference for a first-time player | Medium | High | Tester stalls in Level 5 | **CONFIRMED WORSE THAN EXPECTED** — Level 5 ENERGY face is mathematically unreachable via the hinted-only horizontal path. Level needs redesign. | Level Designer | **Open — Critical** |
| R-004 | Empty automated test coverage slows iteration and hides regressions | Medium | Medium | A later fix breaks an earlier level unnoticed | Test stubs exist; logic not filled (Sprint 2 priority) | Lead Programmer | Open |
